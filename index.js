...
const hard_lvls = [
    ...
    ['e', 'e', 'b-90', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'b-0', 'o', 'e', 'm-270', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'e', 'o', 'm-270', 'e', 'b-90', 'e', 'e', 'b-0', 'e', 'e', 'e', 'e', 'm-90', 'e', 'e', 'e', 'o', 'm-270', 'e', 'e', 'e'],
...
document.querySelector('#back-to-menu-win').addEventListener('click', backToMenu);
document.querySelector('#rules-button-game').addEventListener('click', showRulesG);
document.querySelector('#rules-button-menu').addEventListener('click', showRules);
document.querySelector('#back-to-menu-rules-button').addEventListener('click', hideRules);
document.querySelector('#back-to-game-button').addEventListener('click', hideRulesG);
document.querySelector('#check-button').addEventListener('click', checkCompletion);
document.querySelector('#start-game').addEventListener('click', startGame);
document.querySelector('#button-7x7').addEventListener('click', () => selectDifficulty('7x7'));
document.querySelector('#button-5x5').addEventListener('click', () => selectDifficulty('5x5'));
checkFormValidity();
updateLeaderboard();
...
function startGame() {
    ...
    leaderboard.classList.remove('hidden', 'w-full');
    leaderboard.classList.add('w-1/3');
...
function createGameGrid(size) {
    ...
    dif_lvl[lvl - 1].forEach((s) => {
        let type = s.split('-')[0];
        let orient = parseInt(s.split('-')[1], 10) || 0;
        let newcell;
        switch (type) {
            case 'o':
...
function backToMenu() {
    ...
    document.getElementById('leaderboard').classList.remove('w-1/3');
    document.getElementById('leaderboard').classList.add('w-full');
    lvl = Math.ceil(Math.random() * 5);
}
...
function checkCompletion() {
    const gridSize = selectedDifficulty === '5x5' ? 5 : 7;
    const completion = cells.every(cell => validCell(cell, gridSize));
...
function showWinScreen(time) {
    ...
    winMessage.innerText = `Congratulations, ${playerName}! You completed the railway in ${time}.`;
}
